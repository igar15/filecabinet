delete from document_change_notices;
delete from document_companies;
delete from change_notices;
delete from documents;
delete from developers;
delete from companies;
alter sequence global_seq restart with 1000;

insert into developers (name, chief_name, description, workers_amount)
values ('KTK-40', 'KTK-40 chief name', null, null),
       ('NIO-6', 'NIO-6 chief name', 'Developers from NIO-6 are good folks', 24),
       ('NIO-8', 'NIO-8 chief name', null, 15),
       ('SKB-3', 'SKB-3 chief name', 'SKB-3 is a very good department. There are many talent people work here.', 43),
       ('OTD 25', 'OTD 25 chief name', null, 11),
       ('OTD 33', 'OTD 33 chief name', 'OTD 33 makes great job for the organization!', 22);

insert into companies (name, city, street, building, zipcode)
values ('NTC "NIEMI"', 'Moscow', 'Vereyskaya street', '41', '121357'),
       ('AO "MMZ"', 'Yoshkar-Ola', 'Suvorov street', '15', '424003'),
       ('AO "IEMZ"', 'Izhevsk', 'Pesochnaya street', '3', '426033');

insert into documents (name, decimal_number, inventory_number, receipt_date, status, applicability, form, change_number,
                       stage, sheets_amount, format, a4_amount, developer_id, original_holder_id)
values ('Стойка', 'БА6.151.128', 880572, '2003-01-30', 'ORIGINAL', 'ЮПИЯ.301265.026', 'ELECTRONIC', 3, 'O1', null, null, null, 1000, 1006),
       ('Блок Д02-6М1Э Перечень элементов', 'ВУИА.465211.001ПЭ3', 63140, '2015-03-20', 'ORIGINAL', 'ВУИА.465211.001', 'PAPER', 1, 'O1', 28, 'A4', 28, 1005, 1006),
       ('Крышка', 'ЮПИЯ.301265.026', 884986, '2014-06-30', 'ORIGINAL', null, 'ELECTRONIC', 7, 'O1', null, null, null, 1000, 1006),
       ('Крышка Сборочный чертеж', 'ЮПИЯ.301265.026СБ', 939986, '2014-06-30', 'ORIGINAL', 'ЮПИЯ.301265.026', 'PAPER', 2, 'O1', 1, 'A1', 8, 1000, 1006),
       ('Пульт АРМ ОП СЭ01-7Э Схема электрическая принципиальная', 'ВУИА.468362.021Э3', 68199, '2018-09-15', 'ACC_COPY', 'ВУИА.468362.021', 'ELECTRONIC', 8,  null, null, null, null, null, 1007),
       ('Панель ПСЭ25-22 Перечень элементов', 'ЮПИЯ.468151.031ПЭ3', 929213, '2012-11-18', 'ACC_COPY', 'ЮПИЯ.468151.031', 'ELECTRONIC', 4, null, null, null, null, null, 1008);

insert into change_notices (name, change_code, issue_date, developer_id)
values ('ВУИА.СЭ.739', 5, '2018-05-24', 1000),
       ('ВУИА.СЭ.744', 8, '2018-06-17', 1000),
       ('ВУИА.ТК.133', 4, '2018-08-30', 1002),
       ('ВУИА.ТК.153', 2, '2019-04-24', 1002),
       ('ВУИА.ТК.156', 7, '2020-05-18', 1002),
       ('ВУИА.СО.134', 6, '2016-12-19', 1003),
       ('ВУИА.СО.135', 6, '2018-05-24', 1003),
       ('ВУИА.СО.136', 7, '2018-05-27', 1003);

insert into document_change_notices (document_id, change_notice_id, change)
values (1009, 1015, 1),
       (1009, 1016, 2),
       (1010, 1017, 1),
       (1014, 1017, 1),
       (1010, 1018, 2),
       (1014, 1018, 2),
       (1014, 1019, 3),
       (1011, 1020, 1),
       (1011, 1021, 2),
       (1011, 1022, 3);

insert into document_companies (document_id, company_id)
values (1009, 1007),
       (1009, 1008),
       (1010, 1007),
       (1011, 1008);